...
\begin{abstract}
	...
	\paragraph*{Background:} The Comparative Data Analysis Ontology(CDAO)
...
description of phylogenetic trees and associated character data matrices.
        ...
        \paragraph*{Results:} Using CDAO as a backend triple-store, we developed CDAO-Store. CDAO-Store is a web-based application that can be used to query phylogenetic data for nearest common ancestor or minimum spanning clade as well as filter multiple trees in the store by size, author, taxan,  tree identifier, algorithm or method.  In addition, CDAO-Store also has a visualization application called CDAO-Explorer which can be used to view both the character matrix or tree of anything in CDAO-Store.  In order to keep CDAO-Store up to date, PHYLIP, MEGA, and NEXUS files can be translated to CDAO format and then uploaded to CDAO-Store.

        \paragraph*{Conclusions:} CDAO store is made up of a versatile and compelling set of tools. It provides an good example of what can be done
         with an extensible, semantically aware data resource.
\end{abstract}
...
\section*{Figures}
  \subsection*{Figure 1 - TreeViewer with search}
