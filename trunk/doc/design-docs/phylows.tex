...
any recursive verification of relations so it is also quite efficient.
...
RDFlib module to store the RDF serializations of CDAO instances in a
...
Questions 1,4,6,and 10 can be answered using just SPARQL queries.  Questions
3,5,7,8,and 11 have been implemented using Prolog.   For the queries that are
...
in the source data, and then using xslt we extract the trees associated
with each taxon and then compute the intersection of those sets of 
trees. Ultimately this measure will be replaced as we migrate the existing set of TU
...
Nca is slightly different from msc and as such, is done separately.
...
Where \emph{\$ALGO} is the variable for the given algorithm/software being checked.
...
of accepting arbitrary queries from users, as well as the potential quality
and denial of service issues that this feature will introduce.
...
Not currently implemented but can be implemented using SPARQL by pre-computing and
maintaining these data as new information is added to the system.
...
to our database. We do not store alignment matrices, and handle requests for matrices
using TreeBASE. 

% Matrices can be downloaded as NeXML files from TreeBase, we
...
computers from our Department Computer Lab and averaged them.  Table 1 shows
...
For Local Time, curl wasn't used.  We called the scripts themselves on each of the computers.\\

\begin{tabular}{|c|c|c|c|}
\hline
    & PhlyoWS Time(s) & WebPortal Time(s) & Local Time (s)\\ \hline
Q1  &  2.44              & 1.20                     & 2.02 \\ \hline
Q3a &  1.82           & 3.18                     & 2.51\\ \hline
Q3b &  0.91           & 4.19                     & 1.41\\ \hline
Q4a & 6.12            & 6.18                     & 6.03 \\ \hline
Q4b & 6.19            & 6.26                     & 5.98\\ \hline
Q5  &  32.58          & 35.22                   & 33.21\\ \hline
Q6  & 5.42             & 5.04                   & 4.89\\ \hline
Q7  &   *                & *                            &*\\ \hline
\end{tabular}
\\
\\
...
include keeping meta-data of the tress for some of the queries.  For instance with
...
available would also make it possible to implement this and similar queries with SparQL.
