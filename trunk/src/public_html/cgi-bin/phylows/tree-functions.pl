...
radius( Tree, R ):- findall( E, (leaf(Tree, LeafNode), eccentricity(Tree, E, LeafNode)), Es  ), min_list(Es, R).
radius_count( Tree, _, Count ):- radius(Tree, Count).
...
diameter_count(Tree, _, Count):- diameter( Tree, Count ).
