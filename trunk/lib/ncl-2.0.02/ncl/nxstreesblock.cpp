...
void NxsTreesBlock::HandleTreeDescription(
  ...
  bool utree)		/* true if handling UTREE command, false if handling TREE command */
	...
	if (!token.Equals(";"))
	       {
                 sval += token.GetToken();
		 ///added 3-30 B.C. Skip over if the token is a label. 
                 token.GetNextToken();
...
void NxsTreesBlock::WriteAsNexus(std::ostream &out) const
	...
	if (ntrees > 0)
		...
		if (!translateList.empty())
			{
			out << "\tTRANSLATE" << "\n";
			for (NxsStringMap::const_iterator it = translateList.begin(); it != translateList.end(); ++it)
				{
				if (it != translateList.begin())
					out << ",\n";
