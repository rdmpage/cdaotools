...
class NxsTaxaBlock
  : public NxsBlock
	...
	public:
		...
		class NxsX_NoSuchTaxon {
                        /* added B.C. 4 - 21 - 2008 */
                        public:
                                NxsX_NoSuchTaxon(const NxsString& msg="No Message"):msg_(msg){}
                                const NxsString& GetMessage(){ return msg_; }
                        private:
                          const NxsString& msg_;
                        /* End added section. */
                };	/* thrown if FindTaxon cannot locate a supplied taxon label in the taxonLabels vector */
