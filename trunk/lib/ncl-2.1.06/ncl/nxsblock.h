...
typedef std::pair<const NxsBlock *, std::wstring> BlockUniqueID;
...
class NxsLabelToIndicesMapper
	...
	public:
		...
		virtual unsigned GetIndicesForLabel(const std::wstring &label, NxsUnsignedSet *inds) const = 0;
			/* Returns true if the set replaces an old set*/
		virtual unsigned GetIndexSet(const std::wstring &label, NxsUnsignedSet * toFill) const = 0;
		virtual bool AddNewIndexSet(const std::wstring &label, const NxsUnsignedSet & inds) = 0;
		virtual bool AddNewPartition(const std::wstring &label, const NxsPartition & inds) = 0;

		/// AppendNewLabel is only overloaded in Taxa and State LabelToIndexMappers
		virtual unsigned AppendNewLabel(std::wstring &/*label*/)
			{
			throw NxsUnimplementedException(L"AppendNewLabel called on fixed label interface");
			}
	protected:
		static unsigned GetIndicesFromSets(const std::wstring &label, NxsUnsignedSet *inds, const NxsUnsignedSetMap & itemSets);
		static unsigned GetIndicesFromSetOrAsNumber(const std::wstring &label, NxsUnsignedSet *inds, const NxsUnsignedSetMap & itemSets, const unsigned maxInd, const wchar_t* itemType);
	};

class NxsSetVectorItemValidator;

std::wstring GetBlockIDTitleString(NxsBlock &);
/*----------------------------------------------------------------------------------------------------------------------
...
class NxsBlock
	...
	public:
		...
		virtual void 				WriteBlockIDCommand(std::wostream &out) const;
		virtual void 				WriteLinkCommand(std::wostream &out) const;
		virtual void 				WriteTitleCommand(std::wostream &out) const;
		std::wstring GetTitle() const
			{
			return title;
			}
		void SetTitle(const std::wstring &t, bool autogeneratedTitle)
			{
			title = t;
		...
		*/
		...
		unsigned			ReadVectorPartitionDef(NxsPartition &np, NxsLabelToIndicesMapper &ltm, const std::wstring & partName, const wchar_t* ptype, const wchar_t* cmd, NxsToken & token, bool warnAsterisked, bool demandAllInds, NxsSetVectorItemValidator & v);
		void 				ReadPartitionDef(NxsPartition &np, NxsLabelToIndicesMapper &ltm, const std::wstring & partName, const wchar_t* ptype, const wchar_t* cmd, NxsToken & token, bool warnAsterisked, bool demandAllInds, bool storeAsPartition);
		virtual bool		TolerateEOFInBlock() const
			{
			return false;
			}
		void 				WarnDangerousContent(const std::wstring &s, const NxsToken &t);
		void 				WarnDangerousContent(const std::wstring &s, const ProcessedNxsToken &t);

		void				WriteBasicBlockCommands(std::wostream & out) const;
		virtual void		WriteSkippedCommands(std::wostream & out) const;
	...
	protected:
		...
		void				DemandEndSemicolon(NxsToken &token, const wchar_t*contextString) const;
		void				DemandEquals(NxsToken &token, const wchar_t*contextString) const;
		void				DemandEquals(ProcessedNxsCommand::const_iterator &tokIt, const ProcessedNxsCommand::const_iterator & endIt, const wchar_t*contextString) const ;
		void				DemandIsAtEquals(NxsToken &token, const wchar_t*contextString) const;
		unsigned 			DemandPositiveInt(NxsToken &token, const wchar_t*contextString) const;
		void				GenerateNxsException(NxsToken &token, const wchar_t*message = NULL) const;
		void				GenerateUnexpectedTokenNxsException(NxsToken &token, const wchar_t*expected = NULL) const;
		...
		NxsString			id;					/* holds name of block (e.g., L"DATA", L"TREES", etc.) */
		std::wstring			title;				/* holds the title of the block empty by default */
		std::wstring 		blockIDString; 		/* Mesquite generates these. I don't know what they are for */
		bool				linkAPI;
...
class NxsBlockFactory
	...
	public:
		...
		virtual NxsBlock  *	GetBlockReaderForID(const std::wstring & id, NxsReader *reader, NxsToken *token) = 0;
...
*/
inline void NxsBlock::DemandEquals(NxsToken &token, const wchar_t*contextString) const
	{
	token.GetNextToken();
